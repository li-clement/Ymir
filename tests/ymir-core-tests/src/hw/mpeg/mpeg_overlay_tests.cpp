...
TEST_CASE("MPEG video overlay honours the $A1 EXBG display window for Moon Cradle",
    ...
    // The programmed display coordinates are used directly by the software
    // renderer. Lunar and Vatlva use full-screen windows whose coordinates
    // must remain stable at the framebuffer origin.
    const sint32 exbgDX = window.dispPosX;
    const sint32 exbgDY = window.dispPosY;
...
TEST_CASE("MPEG video overlay with $A1 window clips to the screen and leaves the rest transparent",
          "[mpeg][movie-card][overlay][window]") {
    constexpr uint32 kSrcW = 80;
    constexpr uint32 kSrcH = 80;
    mpeg::DecodedVideoFrame frame{};
    frame.width = kSrcW;
    frame.height = kSrcH;
    ...
    // After the programmed coordinates are applied directly, the visible
    // rectangle starts at X=280 and is clipped at the right screen edge.
    // Pixels outside the visible rectangle stay untouched.
    for (uint32 y = 0; y < kFbH; ++y) {
        for (uint32 x = 0; x < kFbW; ++x) {
            const bool inside = (x >= 280 && x < 320 && y >= 200 && y < 240);
            if (!inside) {
                CHECK(fb[y * kFbW + x] == 0xFF112233u);
            }
        }
    }
    // Inside the visible rectangle (40x40 corner): pixels are written.
    CHECK(fb[200 * kFbW + 280] == 0xFFAABBCCu);
