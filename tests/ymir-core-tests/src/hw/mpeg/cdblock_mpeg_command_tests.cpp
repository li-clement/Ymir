...
TEST_CASE("CD Block $A1 MpegSetWindow latches the EXBG display window for Moon Cradle",
    ...
    h.RunCommand(0xA100, 0x0020, 0x0028, 0x0000);
    ...
    h.RunCommand(0xA101, 0x8011, 0x8011, 0x0000);
    ...
    h.RunCommand(0xA102, 0x000F, 0x0028, 0x0000);
    ...
    h.RunCommand(0xA103, 0x0120, 0x00A0, 0x0000);
    ...
    h.RunCommand(0xA100, 0x0060, 0x003C, 0x0000);
    h.RunCommand(0xA102, 0x0017, 0x0031, 0x0000);
    h.RunCommand(0xA103, 0x00A0, 0x0078, 0x0000);
    ...
    // MpegInit resets the window back to its initial (unconfigured) state.
    h.RunCommand(0x9300);
    CHECK(h.cdb.GetMpegWindowDispSizeW() == 0);
    CHECK(h.cdb.GetMpegWindowDispSizeH() == 0);
}
