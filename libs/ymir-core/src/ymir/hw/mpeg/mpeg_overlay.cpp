...
void MPEGVideoOverlay::BlitLatestFrame(const MPEGCard &card, std::span<uint32> framebuffer, uint32 fbWidth,
    ...
    const auto blitFullFrame = [&] {
        // No usable window configured: 1:1 full-frame blit, clipped to the
        // VDP2 framebuffer. This is also the compatibility path for Lunar
        // and Vatlva when their legacy $A1 values do not describe the decoded
        // frame dimensions.
    ...
    // Reject malformed/stale window state before entering the clipped path.
    // A window whose source rectangle cannot intersect the decoded frame
    // otherwise produces a completely black FMV while audio continues.
    const uint32 srcStepX = DecodeRatio(window->fbRatioX);
    const uint32 srcStepY = DecodeRatio(window->fbRatioY);
    const sint32 sourceLastX = static_cast<sint32>(window->fbPosX) +
                               ((static_cast<sint32>(window->dispSizeW) - 1) * static_cast<sint32>(srcStepX)) / 1000;
    const sint32 sourceLastY = static_cast<sint32>(window->fbPosY) +
                               ((static_cast<sint32>(window->dispSizeH) - 1) * static_cast<sint32>(srcStepY)) / 1000;
    if (window->fbPosX < 0 || window->fbPosY < 0 || sourceLastX < 0 || sourceLastY < 0 ||
        sourceLastX >= static_cast<sint32>(frame.width) || sourceLastY >= static_cast<sint32>(frame.height)) {
        blitFullFrame();
        return;
    }
    ...
    // position/size/ratio.
    //
    // Display position/size are already expressed in the software renderer's
    // framebuffer coordinate space for the supported titles. Keep the
    // programmed coordinates unchanged: Lunar and Vatlva rely on their
    // full-screen $A1 windows landing at the framebuffer origin. Applying
    // an additional raster-origin correction here moves those windows and
    // can leave their decoded picture outside the visible output.
    const sint32 dispX = static_cast<sint32>(window->dispPosX);
    const sint32 dispY = static_cast<sint32>(window->dispPosY);
